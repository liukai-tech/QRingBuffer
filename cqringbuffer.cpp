...
/*!
    ...
    the same QByteArray
*/
const char *CQRingBuffer::readPointerAtPosition(qint64 pos, qint64 &length) const
{
    if (pos >= 0) {
        pos += head;
...
void CQRingBuffer::free(qint64 bytes)
{
...
char *CQRingBuffer::reserve(qint64 bytes)
{
...
*/
char *CQRingBuffer::reserveFront(qint64 bytes)
{
    if (bytes <= 0 || bytes >= MaxByteArraySize)
        return 0;
...
void CQRingBuffer::chop(qint64 bytes)
{
    Q_ASSERT(bytes <= bufferSize);
...
qint64 CQRingBuffer::indexOf(char c, qint64 maxLength, qint64 pos) const
{
...
qint64 CQRingBuffer::read(char *data, qint64 maxLength)
{
...
*/
qint64 CQRingBuffer::peek(char *data, qint64 maxLength, qint64 pos) const
{
    qint64 readSoFar = 0;
...
*/
void CQRingBuffer::append(const char *data, qint64 size)
{
...
*/
void CQRingBuffer::append(const QByteArray &qba)
{
    if (tail == 0) {
        if (buffers.isEmpty())
            buffers.append(qba);
...
qint64 CQRingBuffer::readLine(char *data, qint64 maxLength)
{
    if (!data || --maxLength <= 0)
        return -1;
